...
```{r}
#Create a df that includes the PCA results and group information
pca_df <- as.data.frame(PCA$x)

#Before adding the groups column to this df, always make sure the order is the same. Or use merge()
pca_df$Group <- groups
...
#result output
resultsNames(dds)
DEG <- results(dds) #(differentially expressed) genes
...
SigUP <- DEG[DEG$pvalue < 0.05 & DEG$log2FoldChange > 1,] #change to 0.26 for 1.2x FC or 0.58 for 1.5x FC
write.csv(SigUP, file = "SigUP_LPvsHP.csv")
SigDOWN <- DEG[DEG$pvalue < 0.05 & DEG$log2FoldChange < -1,] #change to 0.58 for 1.5 FC or 0.58 for 1.5x FC
...
with(subset(DEG, padj<0.05 & log2FoldChange < -1), points(log2FoldChange, -log10(pvalue), pch=20, col="blue")) #change cutoff to -0.26 for a logFC of 1.2, instead of 2
with(subset(DEG, padj<0.05 & log2FoldChange > 1), points(log2FoldChange, -log10(pvalue), pch=20, col="red")) #change cutoff to 0.26 for a logFC of 1.2, instead of 2
```
...
```{r}
PCA_n <- prcomp(t(dds_n))
pca_df_n <- as.data.frame(PCA$x)
pca_df_n$Group <- groups
autoplot(PCA, data=pca_df_n, label = TRUE, label.size = 3, colour = "Group")
```
